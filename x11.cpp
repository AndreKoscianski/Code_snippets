...
/*
   ...
   st = XAllocNamedColor(display, colormap, "blue", &blue, &blue);
   if (st == 0) {
...
*/
   ...
   cor.green = 127 << 8;
   cor.blue  = 0;
   XAllocColor(display, DefaultColormap(display,0), &cor);
   ...
   while (1) {
      ...
      if (e.type == Expose) {
         ...
         XSetForeground(display, DefaultGC(display, screen), cor.pixel);
         XFillRectangle(display, win, DefaultGC(display, screen), 10, 60, 10, 10); 
      }
   }
